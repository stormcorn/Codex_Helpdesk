...
export function useMembers(options: UseMembersOptions) {
  ...
  const { feedback: membersFeedback, clearFeedback: clearMembersFeedback } = useTextFeedback();
  ...
  async function deleteMember(member: Member): Promise<void> {
    ...
    try {
      const response = await fetch(`/api/admin/members/${member.id}`, { method: 'DELETE', headers: options.authHeaders() });
      if (!response.ok) {
